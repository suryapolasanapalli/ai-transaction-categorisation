...
class FeedbackAgent:
    ...
    def __init__(self, llm: AzureOpenAI):
        ...
        # Create Agno Agent with feedback processing instructions
        self.agent = Agent(
    ...
    def execute(
        ...
        - Category: {original_classification.get('category', 'Unknown')}
        - Subcategory: {original_classification.get('subcategory', 'Unknown')}
        - Confidence: {original_classification.get('confidence', 'Unknown')}
        - MCC Code: {original_classification.get('mcc_code', 'Unknown')}
        - Description: {original_classification.get('merchant_name', 'Unknown')}
        - Amount: ${original_classification.get('amount', 0)}
        ...
        Provide your response in JSON format with all required fields.
        """
        ...
        try:
            ...
            updated_classification['original_category'] = original_classification.get('category')
            updated_classification['original_subcategory'] = original_classification.get('subcategory')
            
            # If no update needed, preserve original values
            if not updated_classification.get('updated', False):
                updated_classification['category'] = original_classification.get('category')
                updated_classification['subcategory'] = original_classification.get('subcategory')
